...
def test_compare_state_probs_with_discrete( data_num, data_len,dthmm ):
    ...
    t, e = create_data_dt( dthmm, (data_num,data_len) )
...
def test_growing_likelihood(h_states, o_symbols, data_num, data_len):
    ...
    t, e = create_data_ct( cthmm, ( data_num, data_len ) )
...
def create_data_ct( cthmm, size ):
    """Create output variables and time sequences generated by cthmm
       size = ( number of sequences, length of sequences  )
    """
    e = numpy.empty( size, dtype=int )
    t = numpy.empty( size, dtype=int )
    for i in range( size[0] ):
        _,t[i],e[i] = cthmm.generate( size[1] )
    return (t,e)

def create_data_dt( dthmm, size ):
